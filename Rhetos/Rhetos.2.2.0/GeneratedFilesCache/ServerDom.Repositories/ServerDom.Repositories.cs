...
namespace Common
{
    ...
    public static class Infrastructure
    {
        public static readonly RegisteredInterfaceImplementations RegisteredInterfaceImplementationName = new RegisteredInterfaceImplementations
        {
            ...
            { typeof(Rhetos.AspNetFormsAuth.IPasswordAttemptsLimit), @"Common.AspNetFormsAuthPasswordAttemptsLimit" },
            { typeof(Rhetos.AspNetFormsAuth.IPasswordStrength), @"Common.AspNetFormsAuthPasswordStrength" },
    ...
    public class AutofacModuleConfiguration : Autofac.Module
    {
        protected override void Load(Autofac.ContainerBuilder builder)
        {
            ...
            builder.RegisterType<Common._Helper.Permission_Repository>().Keyed<IRepository>("Common.Permission").InstancePerLifetimeScope();
            builder.RegisterType<Common._Helper.AspNetFormsAuthPasswordAttemptsLimit_Repository>().Keyed<IRepository>("Common.AspNetFormsAuthPasswordAttemptsLimit").InstancePerLifetimeScope();
            builder.RegisterType<Common._Helper.AspNetFormsAuthPasswordStrength_Repository>().Keyed<IRepository>("Common.AspNetFormsAuthPasswordStrength").InstancePerLifetimeScope();
            ...
            builder.RegisterType<Common._Helper.AspNetFormsAuthPasswordAttemptsLimit_Repository>().As<IQueryableRepository<Rhetos.AspNetFormsAuth.IPasswordAttemptsLimit>>().InstancePerLifetimeScope();
            builder.RegisterType<Common._Helper.AspNetFormsAuthPasswordStrength_Repository>().As<IQueryableRepository<Rhetos.AspNetFormsAuth.IPasswordStrength>>().InstancePerLifetimeScope();
...
namespace Common._Helper
{
    ...
    public class _ModuleRepository
    {
        ...
        private Permission_Repository _Permission_Repository;
        public Permission_Repository Permission { get { return _Permission_Repository ?? (_Permission_Repository = (Permission_Repository)Rhetos.Extensibility.NamedPluginsExtensions.GetPlugin(_repositories, @"Common.Permission")); } }

        private AspNetFormsAuthPasswordAttemptsLimit_Repository _AspNetFormsAuthPasswordAttemptsLimit_Repository;
        public AspNetFormsAuthPasswordAttemptsLimit_Repository AspNetFormsAuthPasswordAttemptsLimit { get { return _AspNetFormsAuthPasswordAttemptsLimit_Repository ?? (_AspNetFormsAuthPasswordAttemptsLimit_Repository = (AspNetFormsAuthPasswordAttemptsLimit_Repository)Rhetos.Extensibility.NamedPluginsExtensions.GetPlugin(_repositories, @"Common.AspNetFormsAuthPasswordAttemptsLimit")); } }

        private AspNetFormsAuthPasswordStrength_Repository _AspNetFormsAuthPasswordStrength_Repository;
        public AspNetFormsAuthPasswordStrength_Repository AspNetFormsAuthPasswordStrength { get { return _AspNetFormsAuthPasswordStrength_Repository ?? (_AspNetFormsAuthPasswordStrength_Repository = (AspNetFormsAuthPasswordStrength_Repository)Rhetos.Extensibility.NamedPluginsExtensions.GetPlugin(_repositories, @"Common.AspNetFormsAuthPasswordStrength")); } }
...
if (deletedIds.Count() > 0)
        ...
        , IWritableRepository<Common.Principal>, IValidateRepository/*DataStructureInfo RepositoryInterface Common.Principal*/
    {
        ...
        public void Save(IEnumerable<Common.Principal> insertedNew, IEnumerable<Common.Principal> updatedNew, IEnumerable<Common.Principal> deletedIds, bool checkUserPermissions = false)
        {
            ...
            catch (System.Data.Entity.Infrastructure.DbUpdateException saveException)
            {
                ...
                if (interpretedException is Rhetos.UserException && Rhetos.Utilities.MsSqlUtility.IsUniqueError(interpretedException, @"Common.Principal", @"IX_Principal_AspNetUserId"))
                    ((Rhetos.UserException)interpretedException).SystemMessage = @"DataStructure:Common.Principal,Property:AspNetUserId";
        ...
        public IEnumerable<Rhetos.Dom.DefaultConcepts.InvalidDataMessage> Validate(IList<Guid> ids, bool onSave)
        {
            if (onSave)
            {
                var errorIds = this.Filter(this.Query(ids), new SystemRequiredAspNetUserId()).Select(item => item.ID).ToArray();
                if (errorIds.Count() > 0)
                    foreach (var error in GetErrorMessage_SystemRequiredAspNetUserId(errorIds))
                        yield return error;
            }
        ...
        public IEnumerable<InvalidDataMessage> GetErrorMessage_SystemRequiredAspNetUserId(IEnumerable<Guid> invalidData_Ids)
        {
            const string invalidData_Description = @"System required property {0} is not set.";
            IDictionary<string, object> metadata = new Dictionary<string, object>();
            metadata["Validation"] = @"SystemRequiredAspNetUserId";
            metadata[@"Property"] = @"AspNetUserId";
            /*InvalidDataInfo ErrorMetadata Common.Principal.SystemRequiredAspNetUserId*/
            return invalidData_Ids.Select(id => new InvalidDataMessage
            {
                ID = id,
                Message = invalidData_Description,
                MessageParameters = new object[] { @"Integer Common.Principal.AspNetUserId" },
                Metadata = metadata
            });
            // /*InvalidDataInfo OverrideUserMessages Common.Principal.SystemRequiredAspNetUserId*/ return invalidData_Ids.Select(id => new InvalidDataMessage { ID = id, Message = invalidData_Description, Metadata = metadata });
        }

        public IQueryable<Common.Queryable.Common_Principal> Filter(IQueryable<Common.Queryable.Common_Principal> localSource, Common.SystemRequiredAspNetUserId localParameter)
        {
            Func<IQueryable<Common.Queryable.Common_Principal>, Common.DomRepository, Common.SystemRequiredAspNetUserId/*ComposableFilterByInfo AdditionalParametersType Common.Principal.'Common.SystemRequiredAspNetUserId'*/, IQueryable<Common.Queryable.Common_Principal>> filterFunction =
            (source, repository, parameter) => source.Where(item => item.AspNetUserId == null);

            /*ComposableFilterByInfo BeforeFilter Common.Principal.'Common.SystemRequiredAspNetUserId'*/
            return filterFunction(localSource, _domRepository, localParameter/*ComposableFilterByInfo AdditionalParametersArgument Common.Principal.'Common.SystemRequiredAspNetUserId'*/);
        }

        public global::Common.Principal[] Filter(Common.SystemRequiredAspNetUserId filter_Parameter)
        {
            Func<Common.DomRepository, Common.SystemRequiredAspNetUserId/*FilterByInfo AdditionalParametersType Common.Principal.'Common.SystemRequiredAspNetUserId'*/, Common.Principal[]> filter_Function =
                (repository, parameter) => repository.Common.Principal.Filter(repository.Common.Principal.Query(), parameter).ToArray();

            return filter_Function(_domRepository, filter_Parameter/*FilterByInfo AdditionalParametersArgument Common.Principal.'Common.SystemRequiredAspNetUserId'*/);
        }
    ...
    /*DataStructureInfo RepositoryAttributes Common.Permission*/
    public class Permission_Repository : /*DataStructureInfo OverrideBaseType Common.Permission*/ Common.OrmRepositoryBase<Common.Queryable.Common_Permission, Common.Permission> // Common.QueryableRepositoryBase<Common.Queryable.Common_Permission, Common.Permission> // Common.ReadableRepositoryBase<Common.Permission> // global::Common.RepositoryBase
        , IWritableRepository<Common.Permission>, IValidateRepository/*DataStructureInfo RepositoryInterface Common.Permission*/
    {
        private readonly Rhetos.Utilities.ISqlUtility _sqlUtility;
        /*DataStructureInfo RepositoryPrivateMembers Common.Permission*/

        public Permission_Repository(Common.DomRepository domRepository, Common.ExecutionContext executionContext, Rhetos.Utilities.ISqlUtility _sqlUtility/*DataStructureInfo RepositoryConstructorArguments Common.Permission*/)
        {
            _domRepository = domRepository;
            _executionContext = executionContext;
            this._sqlUtility = _sqlUtility;
            /*DataStructureInfo RepositoryConstructorCode Common.Permission*/
        }

        [Obsolete("Use Load() or Query() method.")]
        public override global::Common.Permission[] All()
        {
            return Query().ToSimple().ToArray();
        }

        public override IQueryable<Common.Queryable.Common_Permission> Query()
        {
            /*DataStructureInfo RepositoryBeforeQuery Common.Permission*/
            return _executionContext.EntityFrameworkContext.Common_Permission.AsNoTracking();
        }

        public void Save(IEnumerable<Common.Permission> insertedNew, IEnumerable<Common.Permission> updatedNew, IEnumerable<Common.Permission> deletedIds, bool checkUserPermissions = false)
        {
            if (!DomHelper.CleanUpSaveMethodArguments(ref insertedNew, ref updatedNew, ref deletedIds))
                return;

            /*DataStructureInfo WritableOrm ClearContext Common.Permission*/

            /*DataStructureInfo WritableOrm ArgumentValidation Common.Permission*/

            /*DataStructureInfo WritableOrm Initialization Common.Permission*/

            // Using old data, including lazy loading of navigation properties:
            IEnumerable<Common.Queryable.Common_Permission> deleted = this.Query(deletedIds.Select(item => item.ID)).ToList();
            Rhetos.Utilities.Graph.SortByGivenOrder((List<Common.Queryable.Common_Permission>)deleted, deletedIds.Select(item => item.ID), item => item.ID);
            IEnumerable<Common.Queryable.Common_Permission> updated = this.Query(updatedNew.Select(item => item.ID)).ToList();
            Rhetos.Utilities.Graph.SortByGivenOrder((List<Common.Queryable.Common_Permission>)updated, updatedNew.Select(item => item.ID), item => item.ID);

            /*DataStructureInfo WritableOrm OldDataLoaded Common.Permission*/

            {
                var invalid = insertedNew.Concat(updatedNew).FirstOrDefault(item => item.RoleID == null /*RequiredPropertyInfo OrCondition Common.Permission.Role*/);
                if (invalid != null)
                    throw new Rhetos.UserException("It is not allowed to enter {0} because the required property {1} is not set.",
                        new[] { "Common.Permission", "Role" },
                        "DataStructure:Common.Permission,ID:" + invalid.ID.ToString() + ",Property:RoleID", null);
            }
            {
                var invalid = insertedNew.Concat(updatedNew).FirstOrDefault(item => item.ClaimID == null /*RequiredPropertyInfo OrCondition Common.Permission.Claim*/);
                if (invalid != null)
                    throw new Rhetos.UserException("It is not allowed to enter {0} because the required property {1} is not set.",
                        new[] { "Common.Permission", "Claim" },
                        "DataStructure:Common.Permission,ID:" + invalid.ID.ToString() + ",Property:ClaimID", null);
            }
            {
                var invalid = insertedNew.Concat(updatedNew).FirstOrDefault(item => item.IsAuthorized == null /*RequiredPropertyInfo OrCondition Common.Permission.IsAuthorized*/);
                if (invalid != null)
                    throw new Rhetos.UserException("It is not allowed to enter {0} because the required property {1} is not set.",
                        new[] { "Common.Permission", "IsAuthorized" },
                        "DataStructure:Common.Permission,ID:" + invalid.ID.ToString() + ",Property:IsAuthorized", null);
            }
            /*DataStructureInfo WritableOrm ProcessedOldData Common.Permission*/
            ...
            try
            {
                ...
                {
                    saveOperation = DomHelper.SaveOperation.Insert;
                    _executionContext.EntityFrameworkContext.Common_Permission.AddRange(insertedNew.Select(item => item.ToNavigation()));
                    _executionContext.EntityFrameworkContext.SaveChanges();
                }
            ...
            catch (System.Data.Entity.Infrastructure.DbUpdateException saveException)
            {
                DomHelper.ThrowIfSavingNonexistentId(saveException, checkUserPermissions, saveOperation);
        		Rhetos.RhetosException interpretedException = _sqlUtility.InterpretSqlException(saveException);
        		/*DataStructureInfo WritableOrm OnDatabaseError Common.Permission*/
                if (checkUserPermissions)
                    Rhetos.Utilities.MsSqlUtility.ThrowIfPrimaryKeyErrorOnInsert(interpretedException, "Common.Permission");
            ...
            deleted = null;
            updated = this.Query(updatedNew.Select(item => item.ID));
            IEnumerable<Common.Queryable.Common_Permission> inserted = this.Query(insertedNew.Select(item => item.ID));

            bool allEffectsCompleted = false;
            try
            {
                /*DataStructureInfo WritableOrm OnSaveTag1 Common.Permission*/

                /*DataStructureInfo WritableOrm OnSaveTag2 Common.Permission*/

                Rhetos.Dom.DefaultConcepts.InvalidDataMessage.ValidateOnSave(insertedNew, updatedNew, this, "Common.Permission");
                allEffectsCompleted = true;
            }
            finally
            {
        ...
        public IEnumerable<Rhetos.Dom.DefaultConcepts.InvalidDataMessage> Validate(IList<Guid> ids, bool onSave)
        {
            /*DataStructureInfo WritableOrm OnSaveValidate Common.Permission*/
            yield break;
        }

        /*DataStructureInfo RepositoryMembers Common.Permission*/
    }

    /*DataStructureInfo RepositoryAttributes Common.AspNetFormsAuthPasswordAttemptsLimit*/
    public class AspNetFormsAuthPasswordAttemptsLimit_Repository : /*DataStructureInfo OverrideBaseType Common.AspNetFormsAuthPasswordAttemptsLimit*/ Common.OrmRepositoryBase<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit, Common.AspNetFormsAuthPasswordAttemptsLimit> // Common.QueryableRepositoryBase<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit, Common.AspNetFormsAuthPasswordAttemptsLimit> // Common.ReadableRepositoryBase<Common.AspNetFormsAuthPasswordAttemptsLimit> // global::Common.RepositoryBase
        , IWritableRepository<Common.AspNetFormsAuthPasswordAttemptsLimit>, IValidateRepository/*DataStructureInfo RepositoryInterface Common.AspNetFormsAuthPasswordAttemptsLimit*/
    {
        private readonly Rhetos.Utilities.ISqlUtility _sqlUtility;
        /*DataStructureInfo RepositoryPrivateMembers Common.AspNetFormsAuthPasswordAttemptsLimit*/

        public AspNetFormsAuthPasswordAttemptsLimit_Repository(Common.DomRepository domRepository, Common.ExecutionContext executionContext, Rhetos.Utilities.ISqlUtility _sqlUtility/*DataStructureInfo RepositoryConstructorArguments Common.AspNetFormsAuthPasswordAttemptsLimit*/)
        {
            _domRepository = domRepository;
            _executionContext = executionContext;
            this._sqlUtility = _sqlUtility;
            /*DataStructureInfo RepositoryConstructorCode Common.AspNetFormsAuthPasswordAttemptsLimit*/
        }

        [Obsolete("Use Load() or Query() method.")]
        public override global::Common.AspNetFormsAuthPasswordAttemptsLimit[] All()
        {
            return Query().ToSimple().ToArray();
        }

        public override IQueryable<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit> Query()
        {
            /*DataStructureInfo RepositoryBeforeQuery Common.AspNetFormsAuthPasswordAttemptsLimit*/
            return _executionContext.EntityFrameworkContext.Common_AspNetFormsAuthPasswordAttemptsLimit.AsNoTracking();
        }

        public void Save(IEnumerable<Common.AspNetFormsAuthPasswordAttemptsLimit> insertedNew, IEnumerable<Common.AspNetFormsAuthPasswordAttemptsLimit> updatedNew, IEnumerable<Common.AspNetFormsAuthPasswordAttemptsLimit> deletedIds, bool checkUserPermissions = false)
        {
            if (!DomHelper.CleanUpSaveMethodArguments(ref insertedNew, ref updatedNew, ref deletedIds))
                return;

            /*DataStructureInfo WritableOrm ClearContext Common.AspNetFormsAuthPasswordAttemptsLimit*/

            /*DataStructureInfo WritableOrm ArgumentValidation Common.AspNetFormsAuthPasswordAttemptsLimit*/

            /*DataStructureInfo WritableOrm Initialization Common.AspNetFormsAuthPasswordAttemptsLimit*/

            // Using old data, including lazy loading of navigation properties:
            IEnumerable<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit> deleted = this.Query(deletedIds.Select(item => item.ID)).ToList();
            Rhetos.Utilities.Graph.SortByGivenOrder((List<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit>)deleted, deletedIds.Select(item => item.ID), item => item.ID);
            IEnumerable<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit> updated = this.Query(updatedNew.Select(item => item.ID)).ToList();
            Rhetos.Utilities.Graph.SortByGivenOrder((List<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit>)updated, updatedNew.Select(item => item.ID), item => item.ID);

            /*DataStructureInfo WritableOrm OldDataLoaded Common.AspNetFormsAuthPasswordAttemptsLimit*/

            {
                var invalid = insertedNew.Concat(updatedNew).FirstOrDefault(item => item.MaxInvalidPasswordAttempts == null /*RequiredPropertyInfo OrCondition Common.AspNetFormsAuthPasswordAttemptsLimit.MaxInvalidPasswordAttempts*/);
                if (invalid != null)
                    throw new Rhetos.UserException("It is not allowed to enter {0} because the required property {1} is not set.",
                        new[] { "Common.AspNetFormsAuthPasswordAttemptsLimit", "MaxInvalidPasswordAttempts" },
                        "DataStructure:Common.AspNetFormsAuthPasswordAttemptsLimit,ID:" + invalid.ID.ToString() + ",Property:MaxInvalidPasswordAttempts", null);
            }
            /*DataStructureInfo WritableOrm ProcessedOldData Common.AspNetFormsAuthPasswordAttemptsLimit*/
            ...
            try
            {
                ...
                {
                    saveOperation = DomHelper.SaveOperation.Insert;
                    _executionContext.EntityFrameworkContext.Common_AspNetFormsAuthPasswordAttemptsLimit.AddRange(insertedNew.Select(item => item.ToNavigation()));
                    _executionContext.EntityFrameworkContext.SaveChanges();
                }
            ...
            catch (System.Data.Entity.Infrastructure.DbUpdateException saveException)
            {
                DomHelper.ThrowIfSavingNonexistentId(saveException, checkUserPermissions, saveOperation);
        		Rhetos.RhetosException interpretedException = _sqlUtility.InterpretSqlException(saveException);
        		/*DataStructureInfo WritableOrm OnDatabaseError Common.AspNetFormsAuthPasswordAttemptsLimit*/
                if (checkUserPermissions)
                    Rhetos.Utilities.MsSqlUtility.ThrowIfPrimaryKeyErrorOnInsert(interpretedException, "Common.AspNetFormsAuthPasswordAttemptsLimit");
            ...
            deleted = null;
            updated = this.Query(updatedNew.Select(item => item.ID));
            IEnumerable<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit> inserted = this.Query(insertedNew.Select(item => item.ID));

            bool allEffectsCompleted = false;
            try
            {
                /*DataStructureInfo WritableOrm OnSaveTag1 Common.AspNetFormsAuthPasswordAttemptsLimit*/

                /*DataStructureInfo WritableOrm OnSaveTag2 Common.AspNetFormsAuthPasswordAttemptsLimit*/

                Rhetos.Dom.DefaultConcepts.InvalidDataMessage.ValidateOnSave(insertedNew, updatedNew, this, "Common.AspNetFormsAuthPasswordAttemptsLimit");
                allEffectsCompleted = true;
            }
            finally
            {
        ...
        public IEnumerable<Rhetos.Dom.DefaultConcepts.InvalidDataMessage> Validate(IList<Guid> ids, bool onSave)
        {
            if (onSave)
            {
                var errorIds = this.Filter(this.Query(ids), new SystemRequiredMaxInvalidPasswordAttempts()).Select(item => item.ID).ToArray();
                if (errorIds.Count() > 0)
                    foreach (var error in GetErrorMessage_SystemRequiredMaxInvalidPasswordAttempts(errorIds))
                        yield return error;
            }
            /*DataStructureInfo WritableOrm OnSaveValidate Common.AspNetFormsAuthPasswordAttemptsLimit*/
            yield break;
        }

        public global::Common.AspNetFormsAuthPasswordAttemptsLimit CreateInstance()
        {
            var _item = new global::Common.AspNetFormsAuthPasswordAttemptsLimit();
            /*DataStructureInfo CreateInstance Common.AspNetFormsAuthPasswordAttemptsLimit*/
            return _item;
        }

        public IEnumerable<InvalidDataMessage> GetErrorMessage_SystemRequiredMaxInvalidPasswordAttempts(IEnumerable<Guid> invalidData_Ids)
        {
            const string invalidData_Description = @"System required property {0} is not set.";
            IDictionary<string, object> metadata = new Dictionary<string, object>();
            metadata["Validation"] = @"SystemRequiredMaxInvalidPasswordAttempts";
            metadata[@"Property"] = @"MaxInvalidPasswordAttempts";
            /*InvalidDataInfo ErrorMetadata Common.AspNetFormsAuthPasswordAttemptsLimit.SystemRequiredMaxInvalidPasswordAttempts*/
            return invalidData_Ids.Select(id => new InvalidDataMessage
            {
                ID = id,
                Message = invalidData_Description,
                MessageParameters = new object[] { @"Integer Common.AspNetFormsAuthPasswordAttemptsLimit.MaxInvalidPasswordAttempts" },
                Metadata = metadata
            });
            // /*InvalidDataInfo OverrideUserMessages Common.AspNetFormsAuthPasswordAttemptsLimit.SystemRequiredMaxInvalidPasswordAttempts*/ return invalidData_Ids.Select(id => new InvalidDataMessage { ID = id, Message = invalidData_Description, Metadata = metadata });
        }

        public IQueryable<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit> Filter(IQueryable<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit> localSource, Common.SystemRequiredMaxInvalidPasswordAttempts localParameter)
        {
            Func<IQueryable<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit>, Common.DomRepository, Common.SystemRequiredMaxInvalidPasswordAttempts/*ComposableFilterByInfo AdditionalParametersType Common.AspNetFormsAuthPasswordAttemptsLimit.'Common.SystemRequiredMaxInvalidPasswordAttempts'*/, IQueryable<Common.Queryable.Common_AspNetFormsAuthPasswordAttemptsLimit>> filterFunction =
            (source, repository, parameter) => source.Where(item => item.MaxInvalidPasswordAttempts == null);

            /*ComposableFilterByInfo BeforeFilter Common.AspNetFormsAuthPasswordAttemptsLimit.'Common.SystemRequiredMaxInvalidPasswordAttempts'*/
            return filterFunction(localSource, _domRepository, localParameter/*ComposableFilterByInfo AdditionalParametersArgument Common.AspNetFormsAuthPasswordAttemptsLimit.'Common.SystemRequiredMaxInvalidPasswordAttempts'*/);
        }

        public global::Common.AspNetFormsAuthPasswordAttemptsLimit[] Filter(Common.SystemRequiredMaxInvalidPasswordAttempts filter_Parameter)
        {
            Func<Common.DomRepository, Common.SystemRequiredMaxInvalidPasswordAttempts/*FilterByInfo AdditionalParametersType Common.AspNetFormsAuthPasswordAttemptsLimit.'Common.SystemRequiredMaxInvalidPasswordAttempts'*/, Common.AspNetFormsAuthPasswordAttemptsLimit[]> filter_Function =
                (repository, parameter) => repository.Common.AspNetFormsAuthPasswordAttemptsLimit.Filter(repository.Common.AspNetFormsAuthPasswordAttemptsLimit.Query(), parameter).ToArray();

            return filter_Function(_domRepository, filter_Parameter/*FilterByInfo AdditionalParametersArgument Common.AspNetFormsAuthPasswordAttemptsLimit.'Common.SystemRequiredMaxInvalidPasswordAttempts'*/);
        }

        /*DataStructureInfo RepositoryMembers Common.AspNetFormsAuthPasswordAttemptsLimit*/
    }

    /*DataStructureInfo RepositoryAttributes Common.AspNetFormsAuthPasswordStrength*/
    public class AspNetFormsAuthPasswordStrength_Repository : /*DataStructureInfo OverrideBaseType Common.AspNetFormsAuthPasswordStrength*/ Common.OrmRepositoryBase<Common.Queryable.Common_AspNetFormsAuthPasswordStrength, Common.AspNetFormsAuthPasswordStrength> // Common.QueryableRepositoryBase<Common.Queryable.Common_AspNetFormsAuthPasswordStrength, Common.AspNetFormsAuthPasswordStrength> // Common.ReadableRepositoryBase<Common.AspNetFormsAuthPasswordStrength> // global::Common.RepositoryBase
        , IWritableRepository<Common.AspNetFormsAuthPasswordStrength>, IValidateRepository/*DataStructureInfo RepositoryInterface Common.AspNetFormsAuthPasswordStrength*/
    {
        private readonly Rhetos.Utilities.ISqlUtility _sqlUtility;
        /*DataStructureInfo RepositoryPrivateMembers Common.AspNetFormsAuthPasswordStrength*/

        public AspNetFormsAuthPasswordStrength_Repository(Common.DomRepository domRepository, Common.ExecutionContext executionContext, Rhetos.Utilities.ISqlUtility _sqlUtility/*DataStructureInfo RepositoryConstructorArguments Common.AspNetFormsAuthPasswordStrength*/)
        {
            _domRepository = domRepository;
            _executionContext = executionContext;
            this._sqlUtility = _sqlUtility;
            /*DataStructureInfo RepositoryConstructorCode Common.AspNetFormsAuthPasswordStrength*/
        }

        [Obsolete("Use Load() or Query() method.")]
        public override global::Common.AspNetFormsAuthPasswordStrength[] All()
        {
            return Query().ToSimple().ToArray();
        }

        public override IQueryable<Common.Queryable.Common_AspNetFormsAuthPasswordStrength> Query()
        {
            /*DataStructureInfo RepositoryBeforeQuery Common.AspNetFormsAuthPasswordStrength*/
            return _executionContext.EntityFrameworkContext.Common_AspNetFormsAuthPasswordStrength.AsNoTracking();
        }

        public void Save(IEnumerable<Common.AspNetFormsAuthPasswordStrength> insertedNew, IEnumerable<Common.AspNetFormsAuthPasswordStrength> updatedNew, IEnumerable<Common.AspNetFormsAuthPasswordStrength> deletedIds, bool checkUserPermissions = false)
        {
            if (!DomHelper.CleanUpSaveMethodArguments(ref insertedNew, ref updatedNew, ref deletedIds))
                return;

            /*DataStructureInfo WritableOrm ClearContext Common.AspNetFormsAuthPasswordStrength*/

            {
                var invalidItem = insertedNew.Concat(updatedNew).Where(newItem => newItem.RegularExpression != null && newItem.RegularExpression.Length > 256).FirstOrDefault();
                if (invalidItem != null)
                    throw new Rhetos.UserException(
                        "Maximum length of property {0} is {1}.",
                        new[] { "AspNetFormsAuthPasswordStrength.RegularExpression", "256" },
                        "DataStructure:Common.AspNetFormsAuthPasswordStrength,ID:" + invalidItem.ID.ToString() + ",Property:RegularExpression",
                        null);
            }
            {
                var invalidItem = insertedNew.Concat(updatedNew).Where(newItem => newItem.RuleDescription != null && newItem.RuleDescription.Length > 256).FirstOrDefault();
                if (invalidItem != null)
                    throw new Rhetos.UserException(
                        "Maximum length of property {0} is {1}.",
                        new[] { "AspNetFormsAuthPasswordStrength.RuleDescription", "256" },
                        "DataStructure:Common.AspNetFormsAuthPasswordStrength,ID:" + invalidItem.ID.ToString() + ",Property:RuleDescription",
                        null);
            }
            /*DataStructureInfo WritableOrm ArgumentValidation Common.AspNetFormsAuthPasswordStrength*/

            /*DataStructureInfo WritableOrm Initialization Common.AspNetFormsAuthPasswordStrength*/

            // Using old data, including lazy loading of navigation properties:
            IEnumerable<Common.Queryable.Common_AspNetFormsAuthPasswordStrength> deleted = this.Query(deletedIds.Select(item => item.ID)).ToList();
            Rhetos.Utilities.Graph.SortByGivenOrder((List<Common.Queryable.Common_AspNetFormsAuthPasswordStrength>)deleted, deletedIds.Select(item => item.ID), item => item.ID);
            IEnumerable<Common.Queryable.Common_AspNetFormsAuthPasswordStrength> updated = this.Query(updatedNew.Select(item => item.ID)).ToList();
            Rhetos.Utilities.Graph.SortByGivenOrder((List<Common.Queryable.Common_AspNetFormsAuthPasswordStrength>)updated, updatedNew.Select(item => item.ID), item => item.ID);

            /*DataStructureInfo WritableOrm OldDataLoaded Common.AspNetFormsAuthPasswordStrength*/

            /*DataStructureInfo WritableOrm ProcessedOldData Common.AspNetFormsAuthPasswordStrength*/
            ...
            try
            {
                ...
                {
                    saveOperation = DomHelper.SaveOperation.Insert;
                    _executionContext.EntityFrameworkContext.Common_AspNetFormsAuthPasswordStrength.AddRange(insertedNew.Select(item => item.ToNavigation()));
                    _executionContext.EntityFrameworkContext.SaveChanges();
                }
            ...
            catch (System.Data.Entity.Infrastructure.DbUpdateException saveException)
            {
                DomHelper.ThrowIfSavingNonexistentId(saveException, checkUserPermissions, saveOperation);
        		Rhetos.RhetosException interpretedException = _sqlUtility.InterpretSqlException(saveException);
        		/*DataStructureInfo WritableOrm OnDatabaseError Common.AspNetFormsAuthPasswordStrength*/
                if (checkUserPermissions)
                    Rhetos.Utilities.MsSqlUtility.ThrowIfPrimaryKeyErrorOnInsert(interpretedException, "Common.AspNetFormsAuthPasswordStrength");
            ...
            deleted = null;
            updated = this.Query(updatedNew.Select(item => item.ID));
            IEnumerable<Common.Queryable.Common_AspNetFormsAuthPasswordStrength> inserted = this.Query(insertedNew.Select(item => item.ID));

            bool allEffectsCompleted = false;
            try
            {
                /*DataStructureInfo WritableOrm OnSaveTag1 Common.AspNetFormsAuthPasswordStrength*/

                /*DataStructureInfo WritableOrm OnSaveTag2 Common.AspNetFormsAuthPasswordStrength*/

                Rhetos.Dom.DefaultConcepts.InvalidDataMessage.ValidateOnSave(insertedNew, updatedNew, this, "Common.AspNetFormsAuthPasswordStrength");
                allEffectsCompleted = true;
            }
            finally
            {
        ...
        public IEnumerable<Rhetos.Dom.DefaultConcepts.InvalidDataMessage> Validate(IList<Guid> ids, bool onSave)
        {
            /*DataStructureInfo WritableOrm OnSaveValidate Common.AspNetFormsAuthPasswordStrength*/
            yield break;
        }

        public global::Common.AspNetFormsAuthPasswordStrength CreateInstance()
        {
            var _item = new global::Common.AspNetFormsAuthPasswordStrength();
            /*DataStructureInfo CreateInstance Common.AspNetFormsAuthPasswordStrength*/
            return _item;
        }

        /*DataStructureInfo RepositoryMembers Common.AspNetFormsAuthPasswordStrength*/
    }
